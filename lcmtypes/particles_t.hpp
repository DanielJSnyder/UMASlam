...
#ifndef __particles_t_hpp__
#define __particles_t_hpp__
...
int64_t particles_t::_computeHash(const __lcm_hash_ptr *p)
{
    const __lcm_hash_ptr *fp;
    for(fp = p; fp != NULL; fp = fp->parent)
        if(fp->v == particles_t::getHash)
            return 0;
    const __lcm_hash_ptr cp = { p, (void*)particles_t::getHash };

    int64_t hash = 0xc48afb43c4cd5590LL +
         particle_t::_computeHash(&cp);
