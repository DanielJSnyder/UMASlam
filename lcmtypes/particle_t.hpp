...
int particle_t::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    ...
    tlen = __double_encode_array(buf, offset + pos, maxlen - pos, &this->likelihood, 1);
    if(tlen < 0) return tlen; else pos += tlen;
...
int particle_t::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    ...
    tlen = __double_decode_array(buf, offset + pos, maxlen - pos, &this->likelihood, 1);
    if(tlen < 0) return tlen; else pos += tlen;
...
int64_t particle_t::_computeHash(const __lcm_hash_ptr *)
{
    int64_t hash = 0x31ca1332f45c13edLL;
